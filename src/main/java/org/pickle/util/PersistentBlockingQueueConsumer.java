  ...
  public final void run() {
    while (!disposed) {
      try {
        ...
        Log.debug("Consumer caught InterruptedException.  Continuing to consume anyway.", e);
      }
    }
  }

  /**
   * Called by the run() loop whenever a new object is taken from the queue.  The subclass
   * implementation should process the object accordingly.  If an error is encountered which
   * should cause the object to remain on the queue, then this method should throw a
   * RuntimeException of any kind.
   *
   * @param object the object taken from the queue
   * @throws RuntimeException if processing failed and the object should remain on the queue
   *                         for a retry
   */
