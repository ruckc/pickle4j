...
/**
 * JdbcTemplate eases the use of JDBC by providing standard strategies for executing SQL statements
 * and queries.  The static methods {@link #executeUpdate} and {@link #executeQuery} handle the actual
 * execution  and cleanup required by the JDBC classes.  They accept a custom JdbcTemplate subclass whose
 * methods are called by the static methods at appropriate points in the exection process.
 *
 * Typically the JdbcTemplate is implemented as an anonymous inner class by the caller:
 * <pre><code>
 * // Count all the Freds.
 * Connection c = ...
...
public class JdbcTemplate {
  ...
  /**
   * Called by JdbcTemplate.executeUpdate() and JdbcTemplate.executeQuery() immediately after the
   * PreparedStatement is created.  This method should assign placeholder values on the statement.
   */
  public void statement(PreparedStatement ps) throws SQLException {
  }
  
  /**
   * Called by JdbcTemplate.executeQuery() with the return value of PreparedStatement.executeQuery().
   * This method's return value will be returned by JdbcTemplate.executeQuery().  The input ResultSet
   * should not be returned since it will be closed by the JdbcTemplate.executeQuery(). 
   */
  public Object results(ResultSet rs) throws SQLException {
    return null;
  }
  ...
  /**
   * Called by JdbcTemplate.executeUpdate() with the return value of PreparedStatement.executeUpdate(),
   * which is the number of rows affected by the insert, update, or delete statement.  This call occurs
   * before the transaction is committed.  If this method throws an exception, the transaction is rolled
   * back.
  ...
  private static void close(ResultSet results, Statement statement) {
    if (results != null) {
      try { results.close(); } catch (SQLException e) {
        Log.warn("A ResultSet could not be closed.", e);
      }
    }
    if (statement != null) {
